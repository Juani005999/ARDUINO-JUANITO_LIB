/// ---------------------------------------------------------------------
///
/// Projet          : 2Leds_1LCD-I2C_1HCSR04_1DHT_1Buzzer
/// Auteur          : Juanito del Pepito
/// Version         : 0.0.0.1
/// Date            : 10/04/2023
/// Description     : Projet témoin
///                     - Tuto montrant l'utilisation des objets JUANITO_LED, JUANITO_LCD_I2C, JUANITO_HCSR04, JUANITO_DHT, JUANITO_PASSIVE_BUZZER
...
///                     - Ne faites pas comme dans cet exemple, et n'utilisez qu'un morceau, 2 max
...
#include <JUANITO_PASSIVE_BUZZER_SONGS.h>
...
void loop() {
  ...
  if (mesureOn)
  {
    ...
    yellowLed.DoubleBlink();                                      // LED jaune double clignotement
    ...
    else if (lastMesure < 100)
    {                                                             // On joue sirene
      buzzer.PlaySong(sirene[0], sireneTempo, sireneSize);
      blueLed.On(true);                                           // On positionne la LED bleue à valeurMax
    }
    // Sinon, si lastMesure < 30cm
    else if (lastMesure < 300)
    {                                                             // On joue tripleBip
      buzzer.PlaySong(tripleBip[0], tripleBipTempo, tripleBipSize);
      blueLed.DoubleBlink();                                      // LED bleue double clignotement
    }
    // Sinon, si lastMesure < 50cm
    else if (lastMesure < 500)
    {                                                             // On joue doubleBip
      buzzer.PlaySong(doubleBip[0], doubleBipTempo, doubleBipSize);
      blueLed.Blink();                                            // LED bleue clignotement
    }
    // Sinon
    else
    {                                                             // On joue bip
      buzzer.PlaySong(bip[0], bipTempo, bipSize);
