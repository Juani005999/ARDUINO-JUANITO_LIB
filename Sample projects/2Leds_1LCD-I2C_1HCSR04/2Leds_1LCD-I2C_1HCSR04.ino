/// ---------------------------------------------------------------------
///
/// Projet          : 2Leds_1LCD-I2C_1HCSR04
/// Auteur          : Juanito del Pepito
/// Version         : 0.0.0.1
/// Date            : 10/04/2023
/// Description     : Projet témoin
///                     - Tuto montrant l'utilisation des objets JUANITO_LED, JUANITO_LCD_I2C, JUANITO_HCSR04
...
void loop() {
  ...
  if (mesureOn)
    lastMesure = hcsr04.Distance();                               // Lecture asynchrone de la distance mesurée (mesure effectuée toute les HCSR04_MEASURE_INTERVAL ms)

  // Actualisation du texte relatif à la mesure de distance
  UpdateDistanceText();
  ...
  if (mesureOn)
  {
    ...
    lcd.DisplayText("ON ", 1, 13);                                // Etat des leds ligne 2, StartIndex indice 13
  }
  else
  {
    // On éteint les LEDs
    ...
    if (lastMesure != -1)
    {
      ...
      lcd.DisplayText("OFF", 1, 13);                                // Etat des leds ligne 2, StartIndex indice 13
    }
    else
    {
  ...
  if (millis() > chronoMesure + HCSR04_MEASURE_INTERVAL)
  {
    ...
    if (lastMesure != -1)
    {
      ...
      else
      {
        ...
        // Si nécessaire, on rajoute des espaces jusqu'a atteindre 10 caractères
        while (mesureString.length() < 10)
          mesureString += " ";
      }
